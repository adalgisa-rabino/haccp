...
public class ChecklistItem : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    ...
    // ORA riceve anche lo Sprite invece di caricarlo da Resources
    public void Setup(string nome, Sprite immagine)
    {
        if (infoText != null)
        {
            infoText.text = nome;
        ...
        if (polaroidImage != null)
        {
            if (immagine != null)
            {
                polaroidImage.sprite = immagine;
            }
            else
            {
                Debug.LogWarning("Sprite Polaroid mancante per: " + nome);
            }
        }
